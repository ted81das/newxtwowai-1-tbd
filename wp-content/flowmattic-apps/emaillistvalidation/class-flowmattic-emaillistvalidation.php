<?php
/**
 * Application Name: Emaillistvalidation
 * Description: Add Emaillistvalidation integration to FlowMattic.
 * Version: 1.0
 * Author: InfiWebs
...
/**
 * Emaillistvalidation integration class.
 *
 * @since 1.0
 */
class FlowMattic_Emaillistvalidation {
	/**
	 * Constructor.
	 *
	 * @access public
	...
	public function __construct() {
		// Enqueue custom view for emaillistvalidation.
		add_action( 'flowmattic_enqueue_views', array( $this, 'enqueue_views' ) );

		flowmattic_add_application(
			'emaillistvalidation',
			array(
				'name'         => esc_attr__( 'Email List Validation', 'flowmattic' ),
				'icon'         => FLOWMATTIC_APP_URL . '/emaillistvalidation/icon.svg',
				'instructions' => esc_attr__( 'Enter your Emaillistvalidation account API Key. Your credentials are stored securely in your site.', 'flowmattic' ),
	...
	public function enqueue_views() {
		wp_enqueue_script( 'flowmattic-app-view-emaillistvalidation', FLOWMATTIC_APP_URL . '/emaillistvalidation/view-emaillistvalidation.js', array( 'flowmattic-workflow-utils' ), FLOWMATTIC_VERSION, true );
	}
	...
	public function verify_email( $fields ) {
		...
		$url = 'https://app.emaillistvalidation.com/api/verifEmail?secret=' . $api_key . '&email=' . urlencode( $email ) . '&timeout=15';
		$ch  = curl_init();
...
new FlowMattic_Emaillistvalidation();
