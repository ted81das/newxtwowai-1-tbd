...
@fs_premium_only /modules/auto-fixer/, /modules/cloud-firewall/, /modules/2fa/, /modules/malware-scanner/, /modules/scheduled-scanner/, /modules/events-logger/, /modules/whitelabel/, /modules/visitor-log/, /modules/wizard/, /includes/php-malware-scanner/, /modules/rename-login/, /modules/malware-scanner/js/wf-sn-ms.js, /js/sn-common.js, /images/flags/, /images/virustotal-icon.svg, /vendor/2amigos/, /vendor/bacon/, /vendor/ip2location/, /languages/mo/
...
if ( function_exists( '\\WPSecurityNinja\\Plugin\\secnin_fs' ) ) {
    ...
    if ( secnin_fs()->is__premium_only() ) {
        if ( secnin_fs()->can_use_premium_code() ) {
            ...
            // Fixes
            include_once WF_SN_PLUGIN_DIR . 'modules/auto-fixer/class-wf-sn-fixes.php';
            // Rename login
            include_once WF_SN_PLUGIN_DIR . 'modules/rename-login/class-wf-sn-rename-login.php';
            // Wizard
            include_once WF_SN_PLUGIN_DIR . 'modules/wizard/class-wf-sn-wizard.php';
            // Visitor log
            include_once WF_SN_PLUGIN_DIR . 'modules/visitor-log/class-wf-sn-visitor-log.php';
            // 2FA
            include_once WF_SN_PLUGIN_DIR . 'modules/2fa/class-wf-sn-2fa.php';
        }
    }
    class Wf_Sn {
        /**
        ...
        public static function init() {
            ...
            if ( current_user_can( 'activate_plugins' ) ) {
                ...
                if ( secnin_fs()->is__premium_only() ) {
                    add_action( 'wp_ajax_sn_sidebar_latest_events', array(__NAMESPACE__ . '\\Wf_Sn', 'ajax_sn_sidebar_latest_events__premium_only') );
                }
        ...
        public static function show_topbar() {
            ...
            if ( secnin_fs()->is__premium_only() ) {
                if ( secnin_fs()->can_use_premium_code() ) {
                    ...
                    if ( file_exists( WF_SN_PLUGIN_DIR . 'modules/whitelabel/class-wf-sn-wl.php' ) ) {
                        require_once WF_SN_PLUGIN_DIR . 'modules/whitelabel/class-wf-sn-wl.php';
                    }
                    if ( class_exists( 'WPSecurityNinja\\Plugin\\wf_sn_wl' ) ) {
                        if ( \WPSecurityNinja\Plugin\Wf_Sn_Wl::is_active() ) {
                            $get_new_icon_url = \WPSecurityNinja\Plugin\Wf_Sn_Wl::get_new_icon_url();
                            // $get_new_menu_icon_url = \WPSecurityNinja\Plugin\Wf_Sn_Wl::get_new_menu_icon_url();
                            if ( $get_new_icon_url ) {
                                $icon_url = $get_new_icon_url;
                            } else {
                                $icon_url = '';
                                // No icon if not set
                            }
                            $menu_title = \WPSecurityNinja\Plugin\Wf_Sn_Wl::get_new_name();
                        }
                    }
                }
            }
                ...
                if ( secnin_fs()->can_use_premium_code() ) {
                    if ( class_exists( __NAMESPACE__ . '\\Wf_Sn_Cs' ) ) {
                        if ( method_exists( __NAMESPACE__ . '\\Wf_Sn_Cs', 'scan_files' ) ) {
                            ...
                            wf_sn_el_modules::log_event( 'security_ninja', 'security_tests', 'Checking core files.' );
                        }
                    }
        ...
        /**
         * ajax_sn_sidebar_latest_events__premium_only.
         *
         * @author  Lars Koudal
         * @since   v0.0.1
         ...
         * @return  void
         */
        public static function ajax_sn_sidebar_latest_events__premium_only() {
            // Verify nonce
            if ( !isset( $_POST['nonce'] ) || !wp_verify_nonce( $_POST['nonce'], 'wf_sn_latest_events' ) ) {
            ...
            global $wpdb;
            // Actions we wanna see // @i8n
            $action_list = array(
                'login_form_blocked_ip'         => __( 'Blocked IP', 'security-ninja' ),
                'blockadminlogin'               => __( 'Blocked "admin" user', 'security-ninja' ),
                'blacklisted_IP'                => __( 'Blacklisted IP', 'security-ninja' ),
                'blocked_ip_banned'             => __( 'IP Banned', 'security-ninja' ),
                'login_error'                   => __( 'Login error', 'security-ninja' ),
                'blocked_ip_suspicious_request' => __( 'Suspicious request', 'security-ninja' ),
                'blocked_ip_country_ban'        => __( 'Country block', 'security-ninja' ),
                'login_denied_banned_IP'        => __( 'Login Denied', 'security-ninja' ),
                'firewall_ip_banned'            => __( 'IP Blocked by Firewall', 'security-ninja' ),
            );
            $placeholders = implode( ', ', array_fill( 0, count( array_keys( $action_list ) ), '%s' ) );
            $last_blocks = $wpdb->get_results( $wpdb->prepare( "SELECT id, timestamp, ip, action FROM {$wpdb->prefix}wf_sn_el WHERE action IN ({$placeholders}) ORDER BY timestamp DESC LIMIT 25;", ...array_keys( $action_list ) ) );
            $output = '';
            if ( $last_blocks ) {
                $output .= '<h4>Latest Firewall Events</h4>';
                $output .= '<ul class="wp-list-group">';
                foreach ( $last_blocks as $failed_login ) {
                    $reason = ( isset( $action_list[$failed_login->action] ) ? $action_list[$failed_login->action] : 'Unknown' );
                    $output .= '<li class="wp-list-group-item">';
                    $output .= '<span title="' . esc_attr( $failed_login->action ) . '">' . esc_html( $reason ) . ' from ' . esc_html( $failed_login->ip ) . '</span><br>';
                    $output .= '<small title="Timestamp: ' . gmdate( 'Y-m-d H:i:s', strtotime( $failed_login->timestamp ) ) . '">' . human_time_diff( time() + get_option( 'gmt_offset' ) * HOUR_IN_SECONDS, strtotime( $failed_login->timestamp ) ) . ' ago</small>';
                    $output .= '</li>';
                }
                $output .= '</ul>';
            }
            // Locally blocked IPs
            $my_banned_ips = array();
            if ( class_exists( '\\WPSecurityNinja\\Plugin\\Wf_sn_cf' ) && method_exists( '\\WPSecurityNinja\\Plugin\\Wf_sn_cf', 'get_banned_ips' ) ) {
                $my_banned_ips = \WPSecurityNinja\Plugin\Wf_sn_cf::get_banned_ips();
            }
            if ( isset( $my_banned_ips ) && is_array( $my_banned_ips ) && count( $my_banned_ips ) ) {
                $output .= '<h4>Locally blocked ips</h4>';
                $output .= '<ul>';
                $steplimit = 10;
                $stepcount = 0;
                foreach ( $my_banned_ips as $banned_ip => $ban_time ) {
                    if ( $stepcount < $steplimit ) {
                        $timediff = human_time_diff( time() + get_option( 'gmt_offset' ) * HOUR_IN_SECONDS, $ban_time );
                        $output .= '<li>' . esc_html( $banned_ip ) . ' Expires: ' . esc_html( $timediff ) . '</li>';
                        ++$stepcount;
                    }
                }
                $output .= '</ul>';
        ...
        public static function do_action_admin_init() {
            ...
            if ( secnin_fs()->is__premium_only() ) {
                if ( secnin_fs()->can_use_premium_code() ) {
                    $target_admin_url = 'admin.php?page=security-ninja-wizard';
                }
            }
            ...
            if ( secnin_fs()->is__premium_only() ) {
                if ( secnin_fs()->can_use_premium_code() ) {
                    if ( isset( $_POST['wf-sn-install-routines'] ) ) {
                        if ( check_admin_referer( 'wf-sn-install-routines', '_wpnonce_install_nonce' ) ) {
                            // Perform your installation routines here.
                            $classes_to_activate = array(
                                'Wf_Sn_El'         => esc_html__( 'Events module', 'security-ninja' ),
                                'Wf_Sn_El_Modules' => esc_html__( 'Events extensions', 'security-ninja' ),
                                'Wf_Sn_Cf'         => esc_html__( 'Cloud Firewall', 'security-ninja' ),
                                'Wf_Sn_Ss'         => esc_html__( 'Scheduled Scanner', 'security-ninja' ),
                                'Wf_Sn_Ms'         => esc_html__( 'Malware Scanner', 'security-ninja' ),
                                'Wf_Sn_Vu'         => esc_html__( 'Vulnerability Scanner', 'security-ninja' ),
                            );
                            $logging_class = __NAMESPACE__ . '\\Wf_Sn_El_Modules';
                            foreach ( $classes_to_activate as $class_name => $desc ) {
                                $full_class_name = __NAMESPACE__ . '\\' . $class_name;
                                if ( class_exists( $full_class_name ) ) {
                                    $full_class_name::activate();
                                    if ( method_exists( $logging_class, 'log_event' ) ) {
                                        $logging_class::log_event( 'security_ninja', 'update_routines', __( 'Ran activation routines for', 'security-ninja' ) . ' ' . esc_html( $desc ) );
                                    }
                                }
                            }
                        } else {
                            wp_die( esc_html__( 'Security check failed.', 'security-ninja' ) );
                        }
                    }
                    if ( isset( $_POST['secnin-reset-2fa'] ) ) {
                        if ( check_admin_referer( 'wf-sn-install-routines', '_wpnonce_install_nonce' ) ) {
                            // check user capabilities
                            if ( !current_user_can( 'manage_options' ) ) {
                                wp_die( esc_html__( 'Permission denied.', 'security-ninja' ) );
                            }
                            global $wpdb;
                            $wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->usermeta} WHERE meta_key LIKE %s", 'secnin_2fa_%' ) );
                            \WPSecurityNinja\Plugin\Wf_Sn_El_Modules::log_event( 'security_ninja', '2fa', 'Reset 2FA for all users.' );
                        } else {
                            wp_die( esc_html__( 'Security check failed.', 'security-ninja' ) );
                        }
                    }
        ...
        public static function wpsn_dashboard_widget_render() {
            ...
            if ( secnin_fs()->is__premium_only() ) {
                if ( secnin_fs()->can_use_premium_code() ) {
                    ...
                    if ( $blocked_count ) {
                        ...
                        if ( !empty( $banned_ips ) ) {
                            echo '<h4>Locally blocked ips</h4><ul>';
                            $steplimit = 10;
                            $stepcount = 0;
                            foreach ( $banned_ips as $banned_ip => $ban_time ) {
                                if ( $stepcount < $steplimit ) {
                                    $timediff = esc_html( human_time_diff( current_time( 'timestamp' ), $ban_time ) );
                                    printf( '<li>%s Expires: %s</li>', esc_html( $banned_ip ), esc_html( $timediff ) );
                                    ++$stepcount;
                                }
                            }
                            echo '</ul>';
            ...
            if ( secnin_fs()->is__premium_only() ) {
                if ( secnin_fs()->can_use_premium_code() ) {
                    ...
                    $table_name = $wpdb->prefix . 'wf_sn_el';
                    $query = $wpdb->prepare(
                        "SELECT * FROM `{$table_name}` WHERE `action` IN (%s, %s, %s, %s, %s) AND `module` = %s ORDER BY `timestamp` DESC LIMIT 7",
                        'blocked_ip',
                        'blocked_ip_banned',
                        'banned_ip',
                        'blacklisted_IP',
                    ...
                    $latestevents = $wpdb->get_results( $query );
                    if ( is_array( $latestevents ) && count( $latestevents ) > 0 ) {
                        $blocked_count = get_option( 'wf_sn_cf_blocked_count' );
                        if ( $blocked_count ) {
                            ?>

							<h3><span class="dashicons dashicons-warning"></span> <strong>Firewall:
									<span><?php 
                        ...
                        ?>"><?php 
                        esc_html_e( 'Latest Events', 'security-ninja' );
                        ?></a></p>
			<?php 
                    }
                }
        ...
        /**
         * get_country_img_src__premium_only.
         *
         * @author  Lars Koudal
         * @since   v0.0.1
         * @version v1.0.0  Saturday, March 5th, 2022.
         * @access  public static
         * @param   boolean $country Default: false
         * @param   integer $size    Default: 32
         * @return  mixed
         */
        ...
        public static function plugins_loaded() {
            ...
            if ( secnin_fs()->is__premium_only() ) {
                if ( secnin_fs()->can_use_premium_code() ) {
                    // Load premium translation file if it exists
                    $locale = determine_locale();
                    $mo_file_path = WF_SN_PLUGIN_DIR . '/languages/mo/security-ninja-' . $locale . '.mo';
                    if ( file_exists( $mo_file_path ) ) {
                        load_textdomain( 'security-ninja', $mo_file_path );
                    }
                    ...
                    define( 'WF_SN_DB_VERSION', '5.175' );
                    // Check and update to latest db
                    $wf_sn_db_version = get_option( 'wf_sn_installed_db_version', '1.0' );
                    if ( version_compare( $wf_sn_db_version, '5.146' ) < 0 ) {
                        // Check for wp-admin/upgrade.php
                        global $wp_filesystem;
                        // Make sure that the above variable is properly setup.
                        include_once ABSPATH . 'wp-admin/includes/file.php';
                        WP_Filesystem();
                        // Check whether a file/directory exists.
                        $filename = 'wp-admin/upgrade.php';
                        if ( !$wp_filesystem->exists( ABSPATH . $filename ) ) {
                            $org_file = wp_remote_get( 'https://core.trac.wordpress.org/browser/tags/' . get_bloginfo( 'version' ) . '/src/' . $filename . '?format=txt' );
                            if ( !$org_file['body'] ) {
                                wf_sn_el_modules::log_event( 'security_ninja', 'installation_routines', 'Cannot download wp-admin/upgrade.php ' . WF_SN_DB_VERSION );
                            }
                            if ( !$wp_filesystem->put_contents( trailingslashit( ABSPATH ) . $filename, $org_file['body'], FS_CHMOD_FILE ) ) {
                                wf_sn_el_modules::log_event( 'security_ninja', 'installation_routines', 'Cannot restore wp-admin/upgrade.php ' . WF_SN_DB_VERSION );
                            } else {
                                wf_sn_el_modules::log_event( 'security_ninja', 'installation_routines', 'Restored wp-admin/upgrade.php' );
                            }
                        }
                    }
                    // check wp-admin/upgrade.php in 5.146
                    // **** 5.157 - Retroactively updating options to set autoload = false
                    if ( version_compare( $wf_sn_db_version, '5.157' ) < 0 ) {
                        $cleanup = array(
                        ...
                        foreach ( $cleanup as $optname ) {
                            $value = get_option( $optname );
                            if ( $value ) {
                                // Deleting option to ensure autoload is changed
                        ...
                        wf_sn_el_modules::log_event( 'security_ninja', 'installation_routines', 'Updated options to not be autoloaded from installations older than version 5.157' );
                    }
                    // **** 5.160 - Retroactively updating options to set autoload = false
                    if ( version_compare( $wf_sn_db_version, '5.160' ) < 0 ) {
                        delete_option( 'wf_sn_vu_vulns' );
                        wf_sn_el_modules::log_event( 'security_ninja', 'installation_routines', 'Removed vulnerabilities from database to store in a file - Installs older than version 5.160' );
                    }
                    // **** 5.175 - Cleaning wp-config for duplicate definitions
                    if ( version_compare( $wf_sn_db_version, '5.175' ) < 0 ) {
                        global $wp_filesystem;
                        ...
                        // Check if the wp-config.php file exists and is writable.
                        if ( $wp_filesystem->exists( $file_path ) && $wp_filesystem->is_writable( $file_path ) ) {
                            $wp_config_content = $wp_filesystem->get_contents( $file_path );
                            $lines = explode( "\n", $wp_config_content );
                            ...
                            foreach ( $lines as $line ) {
                                if ( preg_match( "/define\\s*\\(\\s*['\"](.*?)['\"]\\s*,/i", $line, $matches ) ) {
                                    ...
                                    // If the constant has already been defined, skip this line.
                                    if ( in_array( $constant_name, $defined_constants, true ) ) {
                                        continue;
                                    }
                                    $defined_constants[] = $constant_name;
                                }
                                // Add line to new lines, whether it's a define statement or not.
                                $new_lines[] = $line;
                            }
                            // Rebuild the file content without the duplicates and write it back.
                            $wp_config_content = implode( "\n", $new_lines );
                            $wp_filesystem->put_contents( $file_path, $wp_config_content, FS_CHMOD_FILE );
                        }
                    }
                    // Fordi vi kørte en opdatering af en eller anden art, sæt til seneste DB version.
                    update_option( 'wf_sn_installed_db_version', WF_SN_DB_VERSION, false );
                }
            }
        }
        ...
        public static function plugin_meta_links( $links, $file ) {
            if ( secnin_fs()->is__premium_only() ) {
                ...
                $support_link = '<a target="_blank" href="https://wpsecurityninja.com/help/" title="' . esc_attr__( 'Get help or create a support ticket', 'security-ninja' ) . '">' . esc_html__( 'Support', 'security-ninja' ) . '</a>';
        ...
        public static function admin_print_footer_scripts() {
            ...
            if ( secnin_fs()->is__premium_only() ) {
                if ( secnin_fs()->can_use_premium_code() ) {
                    $pointer_content .= '<p>' . __( 'Use the Wizard button to get protected in a few minutes.', 'security-ninja' ) . '</p>';
                    $link_to_url = admin_url( 'admin.php?page=security-ninja-wizard' );
                }
            }
            ...
            if ( secnin_fs()->is__premium_only() ) {
                if ( secnin_fs()->can_use_premium_code() ) {
                    add_submenu_page(
                        ...
                        $capability,
                        'wf-sn-fixes',
                        array(__NAMESPACE__ . '\\Wf_Sn_Fixes', 'fixes_page__premium_only')
                    );
                    add_submenu_page(
                        $menu_slug,
                        'Visitors',
                        ...
                        $capability,
                        'wf-sn-visitor-log',
                        array(__NAMESPACE__ . '\\Wf_Sn_Visitor_log', 'live_log_page')
                    );
                    add_submenu_page(
                        $menu_slug,
                        'Tools',
                        ...
                        $capability,
                        'wf-sn-tools',
                        array(__NAMESPACE__ . '\\wf_sn', 'tools_page__premium_only')
                    );
                    add_submenu_page(
                        $menu_slug,
                        'Wizard',
        ...
        /**
         * return_global_options__premium_only.
         *
         * @author  Unknown
         * @since   v0.0.1
         ...
         * @return  void
         */
        public static function return_global_options__premium_only() {
            $globaloptions = array();
            $wf_sn_cf_options = \WPSecurityNinja\Plugin\wf_sn_cf::get_options();
            $globaloptions['firewall'] = $wf_sn_cf_options;
            unset($globaloptions['firewall']['unblock_url']);
            $wf_sn_vu_options = wf_sn_vu::get_options();
            $globaloptions['vulns'] = $wf_sn_vu_options;
            $globaloptions['eventslogger'] = get_option( 'wf_sn_el' );
            $last_mal_run = wf_sn_ms::get_results( 'last_run' );
            $malscan = wf_sn_ms::get_results( 'do_mal_scan' );
            $malcount = ( is_array( $malscan['files'] ) ? count( $malscan['files'] ) : 0 );
            $globaloptions['malwarescanner'] = array(
                'count'    => $malcount,
                'results'  => $malscan,
                'last_run' => $last_mal_run,
            );
            $ss_options = get_option( WF_SN_SS_OPTIONS_KEY );
            $globaloptions['scheduledscanner'] = $ss_options;
            $wl_options = get_option( 'wf_sn_wl' );
            $globaloptions['whitelabel'] = $wl_options;
            $fixes_options = get_option( WF_SN_FIXES_OPTIONS_KEY );
            $globaloptions['fixes'] = $fixes_options;
            return $globaloptions;
        }

        /**
         * tools_page__premium_only.
         *
         * @author  Lars Koudal
         * @since   v0.0.1
         * @version v1.0.0  Wednesday, January 13th, 2021.
        ...
        public static function tools_page__premium_only() {
            ...
            ?>
						...
						<?php 
            $globaloptions = self::return_global_options__premium_only();
            ?>
						<textarea name="export_wf_data" id="export_wf_data" class="large-text" rows="7"><?php 
            echo esc_textarea( wp_json_encode( $globaloptions ) );
            ?></textarea>
						<p>Note: License data is not exported or imported. You have to enter license details before you can import
							settings.</p>
						...
						<form action="admin.php?page=wf-sn-tools" method="post" id="sn-update-database">
            ...
            ?>
							...
							<input type="hidden" name="_wpnonce_install_nonce" value="<?php 
            ...
            ?>
						...
						<h4><?php 
            esc_html_e( 'Reset all users 2FA', 'security-ninja' );
            ?></h4>
						<form action="admin.php?page=wf-sn-tools" method="post" id="sn-update-database">
            ...
            ?>
							<input type="hidden" name="action" value="secnin_reset_2fa">
							<input type="hidden" name="_wpnonce_install_nonce" value="<?php 
            echo esc_attr( wp_create_nonce( 'wf-sn-install-routines' ) );
            ?>">
							<?php 
            ...
            ?>
							<p><?php 
            esc_html_e( 'Resets all users 2FA setup - forcing all users to start the setup process again before being able to log in.', 'security-ninja' );
            ?></p>
						</form>
						...
						<form action="options.php" method="post">
							<?php 
            settings_fields( 'wf_sn_options' );
            ?>
							<h3><?php 
            esc_html_e( 'Remove settings when deactivating', 'security-ninja' );
            ?></h3>
							<?php 
            self::create_toggle_switch( 'wf_sn_options_remove_settings_deactivate', array(
                'saved_value' => self::$options['remove_settings_deactivate'],
                'option_key'  => 'wf_sn_options[remove_settings_deactivate]',
            ) );
            ?>

							<p class="description"><?php 
            esc_html_e( 'If enabled all settings and data will be reset when you deactivate the plugin.', 'security-ninja' );
            ?></p>
                ...
                if ( secnin_fs()->can_use_premium_code() ) {
                    ...
                    if ( isset( $_POST['action'] ) && 'wf_sn_import_settings' === $_POST['action'] ) {
                        ...
                        if ( is_array( $import_wf_data ) ) {
                            if ( isset( $import_wf_data['vulns'] ) && is_array( $import_wf_data['vulns'] ) ) {
                                update_option( 'wf_sn_vu_settings_group', $import_wf_data['vulns'], false );
                            }
                            if ( isset( $import_wf_data['eventslogger'] ) && is_array( $import_wf_data['eventslogger'] ) ) {
                                update_option( 'wf_sn_el', $import_wf_data['eventslogger'], false );
                            }
                            if ( isset( $import_wf_data['malwarescanner'] ) && is_array( $import_wf_data['malwarescanner'] ) ) {
                                update_option( 'wf_sn_ms_options', $import_wf_data['malwarescanner'], false );
                            }
                            if ( isset( $import_wf_data['whitelabel'] ) && is_array( $import_wf_data['whitelabel'] ) ) {
                                update_option( 'wf_sn_wl', $import_wf_data['whitelabel'], false );
                            }
                            if ( isset( $import_wf_data['fixes'] ) && is_array( $import_wf_data['fixes'] ) ) {
                                if ( defined( 'WF_SN_FIXES_OPTIONS_KEY' ) ) {
                                    update_option( WF_SN_FIXES_OPTIONS_KEY, $import_wf_data['fixes'], false );
                                }
                            }
                        }
                        $redirect_user = true;
        ...
        public static function main_page() {
            ...
            if ( function_exists( '\\WPSecurityNinja\\Plugin\\secnin_fs' ) ) {
                ...
                if ( secnin_fs()->is__premium_only() ) {
                    if ( secnin_fs()->can_use_premium_code() ) {
                        // Pro help scout beacon with chat
                        $helpscoutbeacon = '<script type="text/javascript">!function(e,t,n){function a(){var e=t.getElementsByTagName("script")[0],n=t.createElement("script");n.type="text/javascript",n.async=!0,n.src="https://beacon-v2.helpscout.net",e.parentNode.insertBefore(n,e)}if(e.Beacon=n=function(t,n,a){e.Beacon.readyQueue.push({method:t,options:n,data:a})},n.readyQueue=[],"complete"===t.readyState)return a();e.attachEvent?e.attachEvent("onload",a):e.addEventListener("load",a,!1)}(window,document,window.Beacon||function(){});</script><script type="text/javascript">window.Beacon("init", "e7c8f66b-8c2f-472b-bc97-4a408d362ee0")</script>';
                        // Deactivate if whitelabel active
                        if ( class_exists( __NAMESPACE__ . '\\wf_sn_wl' ) && \WPSecurityNinja\Plugin\Wf_Sn_Wl::is_active() ) {
                            $helpscoutbeacon = '';
                        }
                    }
                }
	...
	<?php 
        ...
        public static function run_tests( $return_response = false ) {
            ...
            if ( secnin_fs()->is__premium_only() ) {
                if ( secnin_fs()->can_use_premium_code() ) {
                    if ( !$set_time_limit ) {
                        wf_sn_el_modules::log_event( 'security_ninja', 'security_tests', __( 'Could not set PHP time limit set_time_limit()', 'security-ninja' ) );
                    }
                }
            }
            $loop_count = 1;
        ...
        public static function activate() {
            ...
            if ( secnin_fs()->is__premium_only() ) {
                if ( secnin_fs()->can_use_premium_code() ) {
                    \WPSecurityNinja\Plugin\Wf_sn_el::activate();
                    \WPSecurityNinja\Plugin\wf_sn_el_modules::activate();
                    \WPSecurityNinja\Plugin\wf_sn_cf::activate();
                    \WPSecurityNinja\Plugin\wf_sn_ss::activate();
                    \WPSecurityNinja\Plugin\wf_sn_el_modules::log_event( 'security_ninja', 'installation_routines', 'Completed activation processes.' );
                }
            }
        }
