...
function aiomatic_editable_form_shortcode($atts)
{
    ...
    if(isset($atts) && is_array($atts) && isset($atts['id']) && !empty($atts['id']))
    {
        ...
        else
        {
            ...
            if($type != 'text' && $type != 'image' && $type != 'image-new' && $type != 'image-mid' && $type != 'image-rep' && $type != 'image-ide' && $type != 'image2')
            {
                $type = 'text';
            }
...
function aiomatic_ideogram_image_shortcode($atts) {
	...
	wp_localize_script($name . 'openai-ideogram-image-ajax', 'aiomatic_ideogram_image_ajax_object', array(
		'ajax_url' => admin_url('admin-ajax.php'),
		'nonce' => wp_create_nonce('openai-ajax-nonce'),
		'image_size' => $image_size,
		'image_placeholder' => $image_placeholder,
