...
function aiomatic_return_god_function_assistants($god_mode, $ai, $lead, $dalle, $stable, $midjourney, $replicate, $ideogram, $amazon, $amazon_details, $scraper, $rss, $google, $captions, $royalty, $youtube, $email, $facebook, $facebook_image, $twitter, $instagram, $pinterest, $business, $youtube_community, $reddit, $linkedin, $webhook, $stable_video)
{
    $return_arr = array();
    if ($god_mode === true)
    {
...
function aiomatic_get_ideogram_object()
{
    return new Aiomatic_Query_Function(
...
function aiomatic_get_ai_object()
{
...
function aiomatic_return_god_function()
{
    ...
    if (isset($aiomatic_Chatbot_Settings['god_mode_enable_ideogram']) && trim($aiomatic_Chatbot_Settings['god_mode_enable_ideogram']) == 'on')
    {
        $return_arr[] = array('type' => 'function', 'function' => aiomatic_get_ideogram_object());
    }
    if (isset($aiomatic_Chatbot_Settings['god_mode_enable_ideogram']) && trim($aiomatic_Chatbot_Settings['god_mode_enable_ideogram']) == 'on')
    {
        $return_arr[] = array('type' => 'function', 'function' => aiomatic_get_ideogram_object());
    ...
    if (isset($aiomatic_Chatbot_Settings['god_mode_ai']) && trim($aiomatic_Chatbot_Settings['god_mode_ai']) == 'on')
    {
        $return_arr[] = array('type' => 'function', 'function' => aiomatic_get_ai_object());
    }
