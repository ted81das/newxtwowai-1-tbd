...
async function aiomaticExecuteAction(action, userState, instance, aiomatic_chat_ajax_object, responded, instant_response) {
    if(action && action.type)
    {
        switch (action.type) 
        {
            case 'send_message':
                ...
                aiomaticDisplayBotMessage(action.message, instance, aiomatic_chat_ajax_object, responded, instant_response, 1000);
...
async function aiomaticProcessUserMessage(userId, message, instance, aiomatic_chat_ajax_object, responded, instant_response) 
{
        ...
        if (userState.currentWorkflow !== null) {
            ...
            if (workflow) {
                ...
                if (actionIndex < workflow.actions.length && workflow.actions[actionIndex]) {
                    ...
                    await aiomaticExecuteAction(action, userState, instance, aiomatic_chat_ajax_object, responded, instant_response);
...
function aiomaticDisplayBotMessage(hardresponse, instance, aiomatic_chat_ajax_object, responded, instant_response, waitdelay = 1000)
{
...
function initChatbotAiomatic(instance)
{
    ...
    async function openaichatfunct() 
    {
        ...
        if( jQuery('#aiomatic_message_input'+ instance).length )
        {
            ...
            if(hardcoded_chat !== '')
            {
                ...
                if(fm[0] !== undefined && fm[0] !== null)
                {
                    ...
                    {
                        aiomaticDisplayBotMessage(hardresponse, instance, aiomatic_chat_ajax_object, responded, instant_response, 1000);
                        return;
                    }
                }
            }
        ...
        const { conversationEnded, actionPerformed } = await aiomaticProcessUserMessage(userId, input_text_raw, instance, aiomatic_chat_ajax_object, responded, instant_response);
