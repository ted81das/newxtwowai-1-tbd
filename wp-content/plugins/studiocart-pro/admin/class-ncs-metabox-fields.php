...
class NCS_Cart_Post_Metabox_Fields {
    ...
    public function get_plans($field_or_repeater_id, $product_subfield_id = 'ob_product') {
        if(!isset($_GET['post'])) {
            return;
        }
        $options = array();
        ...
        if ($product_id = get_post_meta($post_id, $field_or_repeater_id, true)) {

            if(is_array($product_id)){
                foreach($product_id as $prod_id){
                    $pid = $prod_id[$product_subfield_id];
