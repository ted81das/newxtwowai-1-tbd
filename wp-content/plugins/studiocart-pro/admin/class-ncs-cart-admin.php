...
<p>Please see the <a href="https://stripe.com/privacy">Stripe Privacy Policy</a> for more details.</p>',
		...
		<div id="sc-order-details" class="sc-product-info postbox-container">
			<div class="postbox">
				...
				<div>
                    ...
                    <?php 
                    if($invoice = $orderClass->invoice_link_html()) {
                        echo ' | ' . $invoice;
                    }
                    if ($order->status === 'paid') {
                        if($receipt = $orderClass->receipt_link_html()) {
                            echo ' | ' . $receipt;
                        }
                        ?>
                        | <a href="#" id="resend-purchase-confirmation-email" data-order-id="<?php echo esc_attr($post->ID); ?>"><?php esc_html_e( 'Resend Purchase Confirmation Email', 'ncs-cart' ); ?></a>
                        <?php
                    }
                    ?>
                </div>
	...
	*/
    ...
    // Stripe Express Payment Enabled Domain

    public function sc_enabled_payment_domain(){ 
		global $wpdb; 
        ...
        try {
            ...
            $paymentMethodDomain = $this->stripe->request('POST', '/v1/payment_method_domains', [
            ...
            if ($paymentMethodDomain) {
                ...
                $domainId = $paymentMethodDomain['id'];

                // Retrieve payment method configurations
                $paymentMethodConfigurations = $this->stripe->request('GET', '/v1/payment_method_configurations', [], []);    

                $paymentMethodId = '';
                foreach ($paymentMethodConfigurations->data as $config) {
                    if (isset($config['name']) && $config['name'] === $paymentConfiguration && $config['active'] == 'true' && $config['parent'] == null) {
                ...
                $this->stripe->request('POST', "/v1/payment_method_configurations/$paymentMethodId", [
                ...
                if ($enabled == 'true') {
                    $this->verify_apple_pay_domain($domain_name, $domainId);
                }
            }
        } catch (\Exception $e) {
    ...
    public function verify_apple_pay_domain($domain_name, $domainId) {
        try {
            ...
            curl_setopt_array($curl, [
                CURLOPT_URL => 'https://stripe.com/files/apple-pay/apple-developer-merchantid-domain-association',
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_HTTPHEADER => ['Authorization: Bearer ' . $this->stripe->getApiKey()],
            ]);
            ...
            if ($content !== false) {
                ...
                $this->stripe->request('POST', '/v1/apple_pay/domains', ['domain_name' => $domain_name], []);
                $response = $this->stripe->request('POST', "/v1/payment_method_domains/$domainId/validate", ['domain_name' => $domain_name], []);
    

                echo ($response['apple_pay']['status'] === 'active') ? 'Domain ownership verified successfully.' : 'Domain ownership verification failed.';
            } else {
                error_log('Failed to fetch the verification file content.');
            }
