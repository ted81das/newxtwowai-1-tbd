...
class NCS_Cart_White_Label {
    ...
    private function maybe_do_white_label() {
        ...
        if ($api_doc_url = get_option('_sc_wl_api_documentation')) {
            add_filter('sc_stripe_subscriptions_documentation_url', function($url) use ($api_doc_url) {
                return $api_doc_url;
            });
        }

        if ($invoice_date_doc_url = get_option('_sc_wl_invoice_date_documentation')) {
            add_filter('sc_invoice_date_documentation_url', function($url) use ($invoice_date_doc_url) {
                return $invoice_date_doc_url;
            });
        }
    }
	...
	public function get_options_list() {
        ...
        $options = array(
            'sc-whitelist-options' => array(
                ...
                'sc_wl_api_documentation' => array(
                    'type'          => 'text',
                    'label'         => esc_html__( 'API Documentation', 'ncs-cart' ),
                    'settings'      => array(
                        'id'            => '_sc_wl_api_documentation',
                        'value' 		=> '',
                        'description' 	=> '',
                    ),
                ),
                'sc_wl_invoice_date_documentation' => array(
                    'type'          => 'text',
                    'label'         => esc_html__( 'Invoice Date Documentation', 'ncs-cart' ),
                    'settings'      => array(
                        'id'            => '_sc_wl_invoice_date_documentation',
                        'value' 		=> '',
                        'description' 	=> '',
                    ),
                ),
