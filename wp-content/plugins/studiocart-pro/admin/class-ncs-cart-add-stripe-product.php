...
class NCS_Cart_Product_Admin {
                ...
                if($plan) {
                    ...
                    if ( $option['product_type'] == "recurring" && $this->stripe && $stripe_product !== false ) {
                        try {
                            ...
                            if ( ( $retrieve_plan->unit_amount != $plan_price_non_decimal ) || 
                                ( $retrieve_plan->recurring->interval != $plan['interval'] ) ||
                                ( $retrieve_plan->recurring->interval_count != $plan['frequency'] ) ||
                                //( $retrieve_plan->recurring->trial_period_days != $plan['trial_days'] ) ||
                                ( $retrieve_plan->metadata->sc_product_id != $post_id ) ||
                                ( $retrieve_plan->product != $stripe_product->id )
                               ) {
                                $plan_id = $this->create_plan($plan, $post_id, $stripe_product);
                                if ($plan_id) { $objects['_sc_pay_options'][$key][$plan['field_name']] = $plan_id; }
                            } else {
            ...
            foreach($objects['_sc_coupons'] as $key => $option){
                ...
                if($this->stripe){
                    try {
                        ...
                        if ( 
                            ($_type == 'percent' && $_stripe_coupon_percent_off != $_amount) || 
                            ($_type == 'fixed' && $_stripe_coupon_amount_off != sc_price_in_cents($_amount, $sc_currency) ) ||
                            ($duration != $_stripe_coupon_duration ) ||
                            ($duration_months != $_stripe_coupon_duration_months )                            
                        ) {
                            $stripe_coupon->delete();
                            $coupon_id = $this->create_coupon__premium_only($_stripe_id, $_amount, $_type, $sc_currency, $key, $duration, $duration_months);
                            if ($coupon_id) { $objects['_sc_coupons'][$key]['stripe_id'] = $coupon_id; }
                        } else {
    ...
    public function get_stripe_product($post_id){
        ...
        if ( !$this->stripe ) {
            return false;
        } else if ( $pid = get_post_meta($post_id, '_sc_stripe_prod_id', true) ) {
            try {
                ...
                if (!isset($product->metadata->sc_product_id)) {
                    $stripe->products->update(
                        $product->id,[
                            'metadata' => ['sc_product_id' => $post_id, 'origin' => get_site_url()]
                        ]
                    );
                } else if ($product->metadata->sc_product_id != $post_id) {
    ...
    public function create_plan($plan, $post_id, $stripe_prod_id){
        ...
        $recurring = array('interval' => $plan['interval'], 'interval_count' => $plan['frequency']);
        ...
        try {	
            $stripe_plan = $stripe->prices->create([
              'unit_amount' => sc_price_in_cents($plan['amount'], $sc_currency), // dollars to cents
    ...
    public function create_coupon__premium_only($stripe_id, $_amount, $_type, $sc_currency, $coupon_group_id, $duration, $duration_months){
        $stripe = $this->stripe;
        try{
            
            $args = [
                'id' => $this->format_coupon_id($stripe_id),
                'name' => $stripe_id,
                'duration' => $duration,
                'currency' => $sc_currency,
            ];
            ...
            if ($duration_months != null) {
                $args['duration_in_months'] = $duration_months;
            }
        
            if ($_type == 'percent') {
                $args['percent_off'] = $_amount;
            } else {
                $args['amount_off'] = sc_price_in_cents($_amount, $sc_currency);
            }
            
            $stripe_coupon = $stripe->coupons->create($args);            
