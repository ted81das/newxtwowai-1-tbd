<?php
    ...
    $is_paid = in_array($order->status, ['paid', 'completed']);
    $is_refunded = ($order->status == 'refunded');
    $document_type = (isset($_GET['type']) && $_GET['type'] == 'receipt') ? esc_attr__('Receipt', 'ncs-cart') : esc_attr__('Invoice', 'ncs-cart');
    $amount_text = ($document_type == 'Receipt') ? esc_attr__('Amount Paid', 'ncs-cart') : esc_attr__('Amount Due', 'ncs-cart');
    ...
    $html .= '<tr class="top">
        <td class="title" style="width: 30%; font-family:'.$font.';">'. $image_file . '<br>'; 
            ...
            <td valign="top" style="width: 70%;text-align: right;clear:both;">
                <p style="font-size: 46px;color: rgb(165,179,183);line-height: 2em;font-family:'.$font.';">'.$document_type.'</p>
                <p>
                    <span style="color:#818d90; font-weight:bold;font-family:'.$font.';">'.esc_attr__($document_type.' Number', 'ncs-cart').'</span>
                    <br/>
        ...
        <tr class="information">
            <td colspan="2">
                <table style="margin-top:20px;" width="100%" cellspacing="0" cellpadding="13">
                    ...
                    foreach($items['items'] as $item) {
                        ...
                        $html .= '<tr>
                                ...
                                if(isset($item['subscription_id'])) {
									...
									$start = sc_maybe_format_date($order->date);
                        ...
                        if(isset($item['subscription_id']) && $item['sign_up_fee'] && $item['sign_up_fee'] > 0) {
                            ...
                            <td style="font-family:'.$font.';'.$bg[$i%2].';border-bottom:0px solid #a5b3b7">'.sc_format_price($item['sign_up_fee'] / $sub->quantity).'</td>
                            <td style="font-family:'.$font.';'.$bg[$i%2].';border-bottom:0px solid #a5b3b7; font-weight: bold;" align="right">'.sc_format_price($item['sign_up_fee']).'</td></tr>';
                            $i++;
                        }
                    }
                    ...
                    <tr>
                        <td colspan="2"></td>
                        <td style="font-family:'.$font.';background:#e2e9eb; border-top:1px solid #fff; border-bottom:0.75px solid #6f7b7e; font-weight: bold;">'.$amount_text.'</td>
                    ...
                    if ($is_refunded) {
                        foreach ($order->refund_log as $refund) {
                            $html .= '<tr>
                                <td colspan="2"></td>
                                <td style="font-family:'.$font.';color: #e74c3c;">'.sprintf(esc_attr__('Refunded on %s', 'ncs-cart'), $refund['date']).'</td>
                                <td style="font-family:'.$font.';color: #e74c3c;" align="right">-'.sc_format_price($refund['amount']).'</td>
                            </tr>';
                        }
                    }
