...
if($event->data->object->object == 'charge') {
    ...
    if (!isset($charge->metadata->sc_product_id) && !isset($charge->invoice)) {
        http_response_code(200);
        exit();
    }
    ...
    }else if(isset($charge->subscription_details) && isset($charge->subscription_details->metadata->origin)){
        if ($charge->subscription_details->metadata->origin != get_site_url()) {
            http_response_code(200);
            exit();
        }
    }
