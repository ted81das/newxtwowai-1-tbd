...
if ( ($data['subscr_id'] || $data['txn_id']) && sc_verifyTransaction($_REQUEST) ) {
    ...
    if (strpos($data['txn_type'], 'subscr_') === 0 && isset($data['subscription_id'])) {
        ...
        if($sub->sub_next_bill_date <= time()) {
            $sub->sub_next_bill_date = sc_get_next_bill_date_paypal($sub);
        }
...
function sc_get_next_bill_date_paypal($sub) {
    ...
    $sc_paypal = new NCS_Cart_Paypal('','');
    $access_token = $sc_paypal->sc_paypal_oauthtoken();
    ...
    $response = wp_remote_get($endpoint, array(
        'headers' => $headers,
        'timeout' => 45 // Set your preferred timeout value
    ));
