...
class NCS_Cart_Paypal extends NCS_Cart_Public {
	...
	public function __construct( $plugin_name, $version ) {
        ...
        add_action('sc_order_refund_paypal', [ $this, 'sc_paypal_refund'],10,2);
        add_filter('sc_cancel_subscription', [ $this, 'sc_paypal_cancel_subscription'], 10, 4);
        add_filter('sc_subscription_pause_restart', [ $this, 'sc_paypal_pause_restart_subscription'], 10, 3);
        add_filter('sc_enabled_payment_gateways', [ $this, 'maybe_add_paypal_enabled']);
        add_filter('sc_payment_methods', [ $this, 'maybe_add_paypal_pay_method'], 10, 2);
    ...
    public function paypal_process_upsell__premium_only() {
        ...
        if ( !isset($_POST['sc-order']) || !isset( $_POST['sc-nonce'] ) ){
            echo json_encode(array(
                'message' => __("There was a problem processing this order, please contact us for assistance.", "ncs-cart"), 
            ));
        }
        ...
        $sub = '';
        $order = ScrtOrder::child_of($_POST['sc-order'], $action);
        ...
        $parentId = intval($_POST['sc-order']);
        $sc_product_id = get_post_meta($parentId, 'sc_product_id', true);
        
        $return_url = apply_filters('studiocart_post_purchase_url', $_POST['return_url'], $parentId, $sc_product_id);
    ...
    public function sc_paypal_refund($data,$order){
        ...
        $amount= array('amount' => array('total'=> $amount, 'currency' => $sc_currency), 'description'=> 'Cancelled in Studiocart');
    ...
    public function sc_paypal_pause_restart_subscription($return, $sub, $type){
        ...
        $sub_args= array('reason'=> $type.' subscription');
        $response = $this->doCurlRequest($paypalUrl, $sub_args);
        return $this->handleCurlResponse($response, $sub ,$status);

    }
  
    public function sc_paypal_cancel_subscription($canceled, $sub, $sub_id, $now=true){
        global $sc_currency;
        ...
        $sub_args= array('reason'=> 'Not satisfied with the service');
    ...
    private function sc_build_paypal_url($order,$sub) {
        ...
        $data = array(
            ...
            'custom'        => json_encode(apply_filters('sc_paypal_custom_payment_vars', array('order_id' => $order->id), $order->get_data(), false)),
        ...
        if(!empty($sub)){
            ...
            $custom = apply_filters('sc_paypal_custom_payment_vars',array(
                        'order_id' => $order->id,
        ...
        $data = apply_filters('sc_paypal_payment_vars', $data, $order,$sub);
   ...
   public function sc_paypal_request() {
       ...
       //error_reporting(E_ALL); 
        ...
        $return_url = apply_filters('studiocart_post_purchase_url', $return_url, $order_id, $sc_product_id);
    ...
    public function sc_paypal_process_payment() {
        ...
        if ( isset($_GET['sc-pp']) && isset($_GET['sc-order'])){
            ...
            if(!empty($paypalPDT) && isset($_GET['tx'])) {
                $paypal_data = array('paypalPDT'=>$paypalPDT,
                                    'tx'=>$_GET['tx'],
                                    'paypalUrl'=>$paypalUrl);
            ...
            $return_url = apply_filters('studiocart_post_purchase_url', $return_url, $order_id, $scp->ID);
    ...
    private function run_pdt_check($order_info, $paypal_data) {
        ...
        CURLOPT_POSTFIELDS => array('cmd' => '_notify-synch',
                                'tx' => $paypal_data['tx'],
