...
use Dompdf\Dompdf;
    ...
    if($order) {
        ...
        try {            
            ...
            $doc_type = isset($_GET['type']) && $_GET['type'] === 'receipt' ? 'receipt' : 'invoice';

            // Set the appropriate file prefix
            $file_prefix = $doc_type === 'receipt' ? esc_html__("receipt", 'ncs-cart') : esc_html__("invoice", 'ncs-cart');

            // Create the filename
            $fileName = $file_prefix . "-" . get_post_timestamp($id) . ".pdf";
            $pdfPath = $fileName;
            $dompdf = new Dompdf();
            $content ="<html>
                <body>
