...
public static function store_items($order) {
  ...
  public function invoice_link($download = true) {
    if($this->status != 'pending-payment' || $this->pay_method=='cod') {
        ...
        return home_url().'?sc-invoice='.$invoice_id.get_post_timestamp($invoice_id).'&id='.$invoice_id.'&dl='.$download.'&type=invoice';
    } else {
        return false;
    }
  }
  ...
  public function receipt_link($download = true) {
    if($this->status == 'paid' || $this->status == 'completed' || $this->status == 'refunded') {
        ...
        return home_url().'?sc-invoice='.$receipt_id.get_post_timestamp($receipt_id).'&id='.$receipt_id.'&dl='.$download.'&type=receipt';
    } else {
        return false;
    }
  }
  ...
  public function receipt_link_html($label = false) {
    if($link = $this->receipt_link()) {
        if ($label === false) {
            $label = esc_html__('Download Receipt', 'ncs-cart');
        }
        return '<a href="'.$link.'" target="_blank" rel="noopener noreferrer">'.$label.'</a>';
    } else {
        return false;
