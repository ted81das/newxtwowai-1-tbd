...
if ( ! class_exists( 'Subscribe' ) ) {
	...
	class Subscribe extends Notice {
		...
		public function __construct() {
			parent::__construct();
			add_action( 'wp_ajax_jlt_admin_bar_editor_subscribe', array( $this, 'jlt_admin_bar_editor_subscribe' ) );
		}
		 ...
		 * @author Jewel Theme <support@jeweltheme.com>
		 */
		public function jlt_admin_bar_editor_subscribe() {
			check_ajax_referer( 'jlt_admin_bar_editor_subscribe_nonce' );
		...
		public function notice_content() {
			...
			?>
			<div class="notice notice-admin-bar is-dismissible notice-plugin-review notice-<?php echo esc_attr( $this->color ); ?> admin-bar-notice-<?php echo esc_attr( $this->get_id() ); ?>">
				<button type="button" class="notice-dismiss admin-bar-notice-dismiss"></button>
				<img width="70" height="70" src="<?php echo esc_url( JLT_ADMIN_BAR_EDITOR_IMAGES . '/subscribe-icon.svg' ); ?>" alt="<?php esc_attr_e( 'Logo', 'admin-bar' ); ?>">
				<div class="admin-bar-subscribe-content">
					<?php $this->set_title(); ?>
					<form style="display:flex; gap: 10px;">
						<div class="admin-bar-plugin-subscribe-input">
						...
						<button type="submit" class="button button-primary admin-bar-subscribe-btn"><?php esc_html_e( 'Get Discount', 'admin-bar' ); ?></button>
					</form>
				</div>
			</div>
			<?php
		...
		public function plugin_rate_url() {
			return 'https://wordpress.org/plugins/' . JLT_ADMIN_BAR_EDITOR_SLUG;
		}
		...
		public function core_script( $trigger_time ) {
			...
			<script>
				jQuery(document).on('submit', '.admin-bar-notice-subscribe .admin-bar-subscribe-content form', function(e) {
					...
					let formWrapper = form.closest('.admin-bar-subscribe-content');
					...
					jQuery.ajax({
							...
							crossDomain: true,
							data: {
								action: 'jlt_admin_bar_editor_subscribe',
								_wpnonce: '<?php echo esc_js( wp_create_nonce( 'jlt_admin_bar_editor_subscribe_nonce' ) ); ?>',
								name: name,
								email: email,
							}
						})
						.done(function(response) {
							formWrapper.hide().after('<p class="admin-bar--notice-message"><strong>' + response.data + '</strong><p>');
							let subsTimeout = setTimeout(function() {
								jlt_admin_bar_editor_notice_action(null, form, 'disable');
