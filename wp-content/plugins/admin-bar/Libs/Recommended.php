...
if ( ! class_exists( 'Recommended' ) ) {
	...
	class Recommended {
		...
		public function __construct( $menu_order = 99 ) {
			...
			add_action( 'wp_ajax_jlt_admin_bar_editor_recommended_upgrade_plugin', array( $this, 'jlt_admin_bar_editor_recommended_upgrade_plugin' ) );
			add_action( 'wp_ajax_jlt_admin_bar_editor_recommended_activate_plugin', array( $this, 'jlt_admin_bar_editor_recommended_activate_plugin' ) );
		}
		...
		public function render_recommended_plugins() { ?>
			<div class='admin-bar-recommended-wrapper'>
				<?php $this->header(); ?>
				<?php $this->body(); ?>
			</div>
			<?php
		...
		public function header() {
			?>
			<div class='admin-bar-recommended-header'>
				<div class='admin-bar-recommended-title'>
					<h2>
						<?php echo esc_html__( 'Recommended Plugins', 'admin-bar' ); ?>
					</h2>
				</div>
				<div class='admin-bar-recommended-menu'>
					<div class="wp-filter">
						...
						<form class="search-form admin-bar-search-plugins mr-0" method="get">
		 ...
		 * @author Jewel Theme <support@jeweltheme.com>
		 */
		public function jlt_admin_bar_editor_recommended_activate_plugin() {
			try {
				if ( isset( $_POST['file'] ) ) {
					$nonce = isset( $_POST['nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['nonce'] ) ) : '';

					if ( ! wp_verify_nonce( $nonce, 'jlt_admin_bar_editor_recommended_nonce' ) ) {
						wp_send_json_error( array( 'mess' => esc_html__( 'Nonce is invalid', 'admin-bar' ) ) );
					}

					if ( ( is_multisite() && ! is_network_admin() ) || ! current_user_can( 'install_plugins' ) ) {
						wp_send_json_error( array( 'mess' => esc_html__( 'Invalid access', 'admin-bar' ) ) );
					}
					...
					if ( ! in_array( $plugin, $plugin_links ) ) {
						wp_send_json_error( array( 'mess' => esc_html__( 'Invalid plugin', 'admin-bar' ) ) );
					}
					...
					wp_send_json_success(
						array(
							'mess' => esc_html__( 'Activate success', 'admin-bar' ),
						)
					);
				}
			} catch ( \Error $ex ) {
				wp_send_json_error(
					array(
						'mess' => esc_html__( 'Error.', 'admin-bar' ),
						array(
							'error' => $ex,
						),
					)
		 ...
		 * @author Jewel Theme <support@jeweltheme.com>
		 */
		public function jlt_admin_bar_editor_recommended_upgrade_plugin() {
			try {
				require_once ABSPATH . 'wp-admin/includes/plugin-install.php';
					...
					if ( 'install' === $type ) {
						...
						if ( $plugin_data && ! is_wp_error( $plugin_data ) ) {
							...
							if ( is_wp_error( $active_plugin ) ) {
								...
								wp_send_json_success(
									array(
										'mess' => esc_html__( 'Install success', 'admin-bar' ),
									)
								);
							}
						} else {
						...
						if ( is_wp_error( $result ) ) {
							...
							wp_send_json_success(
								array(
									'mess'   => esc_html__( 'Update success', 'admin-bar' ),
									'active' => $is_active,
								)
							);
						}
				...
				wp_send_json_error(
					array(
						'mess' => esc_html__( 'Error exception.', 'admin-bar' ),
						array(
							'error' => $ex,
						),
					)
