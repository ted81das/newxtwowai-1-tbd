(function() {
  ...
  function menu(element) {
    ...
    function setMaxWidth() {
      this.style.maxWidth = `${window2.innerWidth}px`;
    }
    ...
    function enableMobileMenu() {
      if (!mobileMenuEnabled) {
        ...
        if (config.mobile_menu_close_on_click) {
          const menuItems = Array.from(element.querySelectorAll("a"));
          console.log(menuItems);
          menuItems.forEach((item) => {
            var _a;
            const hasChild = (_a = item.parentElement) == null ? void 0 : _a.querySelector(".sub-menu");
            if (!hasChild) {
    ...
    function destroy() {
      window2.removeEventListener("resize", onBrowserResize);
      mobileMenuTrigger == null ? void 0 : mobileMenuTrigger.removeEventListener("click", toggleMobileMenu);
      window2.removeEventListener("click", maybePreventDefault);
    }
    ...
    window2.addEventListener("resize", onBrowserResize);
    mobileMenuTrigger == null ? void 0 : mobileMenuTrigger.addEventListener("click", toggleMobileMenu);
    window2.addEventListener("click", maybePreventDefault);
    onBrowserResize();
